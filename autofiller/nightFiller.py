...
def fillerUserData(aula, nome, mail, matricola, telefono,row, column) :
        ...
        try:
                ...
                table = driver.find_element_by_xpath(f"//table[@class='ui-datepicker-calendar']/tbody/tr[1]/td[2]")
                time.sleep(5)
                table.click()
                time.sleep(5)
                print("qui si")
                ...
                clickMatricola = driver.find_element_by_xpath('//*[@id="post-335"]/div/div/div[2]/div/form/div[6]/div[4]/div/input')
                clickTelefono = driver.find_element_by_xpath('//*[@id="post-335"]/div/div/div[2]/div/form/div[6]/div[5]/div/input')
                clickGdpr = driver.find_element_by_xpath('//*[@id="ea-gdpr"]') #xpath corrispondente alla locazione del tasto di accettazione del trattamento dei dati
                clickConferma = driver.find_element_by_xpath('//*[@id="post-335"]/div/div/div[2]/div/form/div[6]/div[9]/div/button[1]')
                time.sleep(5)

                #clear + autofill dei campi (clear con key_down(Keys.CONTROL).send_keys('a') )
                ActionChains(driver).move_to_element(clickMail).click().key_down(Keys.CONTROL).send_keys('a').key_up(Keys.CONTROL).send_keys(mail).perform()
                ActionChains(driver).move_to_element(clickName).click().key_down(Keys.CONTROL).send_keys('a').key_up(Keys.CONTROL).send_keys(nome).perform()
                ActionChains(driver).move_to_element(clickMatricola).click().key_down(Keys.CONTROL).send_keys('a').key_up(Keys.CONTROL).send_keys(matricola).perform()
                ActionChains(driver).move_to_element(clickTelefono).click().key_down(Keys.CONTROL).send_keys('a').key_up(Keys.CONTROL).send_keys(telefono).perform()
                time.sleep(4)
